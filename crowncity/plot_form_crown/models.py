...
class sale_order_plot_extended(models.Model):
	...
	plot_payment_id = fields.One2many('account.invoice','plot_payment_history_tree',string='Payment History')
	...
	# @api.onchange('diff_in_installment')
	# def cul_ins_rate_diff(self):
	#     rate_per_installment = self.rate_per_installment
	#     if self.diff_in_installment:
	#         self.rate_per_installment = rate_per_installment - self.diff_in_installment
	...
	def installment_invoice(self):
		...
		# date_due = start_date + timedelta(days=5)
		# rate_per_installment = self.rate_per_installment
		# for x in xrange(0,self.no_of_installments):
		#     description = "Installment No: "+str(x+1)+" From "+str(start_date)+" To "+str(date_due) 
		#     if self.installment_plan.name == 'monthly':
		#         start_date = start_date + timedelta(days=365/12)
		#         date_due = start_date + timedelta(days=5)
		#         if x == 10 or x == 22:
		#             rate_per_installment = ((self.total_plot_price - ((self.rate_per_installment * 33) + self.plot_rate_marla))/3) - 16.67
		#         else:
		#             rate_per_installment = self.rate_per_installment
		#         if  x == 34:
		#             rate_per_installment = ((self.total_plot_price - ((self.rate_per_installment * 33) + self.plot_rate_marla))/3) + (self.plot_size.plot_difference + 33.33)

		#     elif self.installment_plan == 'quarterly':
		#         start_date = start_date + timedelta(days=3*365/12)
		#         date_due = start_date + timedelta(days=5)
		#     elif self.installment_plan == 'six_months':
		#         start_date = start_date + timedelta(days=6*365/12)
		#         date_due = start_date + timedelta(days=5)
		#     elif self.installment_plan == 'bimonthly':
		#         start_date = start_date + relativedelta(months=+2)
		#         date_due = start_date + timedelta(days=5)
		#         if x == 5 or x == 11:
		#             rate_per_installment = self.total_plot_price * 0.1
		#         else:
		#             rate_per_installment = self.rate_per_installment - 33.33
		#         if x==17:
		#             rate_per_installment = (self.total_plot_price * 0.1) + (500)
		#     elif self.installment_plan == 'lump_sum':
		#         start_date = start_date + timedelta(days=365/12)
		#         date_due = start_date + timedelta(days=5)
		...
		# date_due = start_date + timedelta(days=5)
		# rate_per_installment = self.rate_per_installment
		# if self.plot_size.plot_difference:
		#     for x in xrange(0,self.no_of_installments):
		#         description = "Installment No: "+str(x+1)+" From "+str(start_date)+" To "+str(date_due) 
		#         self._create_plot_invoice(description, rate_per_installment, self.id, self.partner_id.id, start_date , date_due)
		#         self.plot_payment_id = self._prepare_payment_history_line(self.plot_invoice_id.id, self.plot_invoice_id.date_invoice, self.plot_invoice_id.date_due, self.plot_invoice_id.amount_total)
		#         if self.installment_plan == 'monthly':
		#             start_date = start_date + timedelta(days=365/12)
		#             date_due = start_date + timedelta(days=5)
		#             if x == 10 or x == 22:
		#                 rate_per_installment = ((self.total_plot_price - ((self.rate_per_installment * 33) + self.plot_rate_marla))/3) - 16.67
...
class plot_payment_history(models.Model):
	...
	plot_payment_history_tree = fields.Many2one('sale.order')
	...
	def validate(self):
		...
		if self.plot_number or self.plot_file_no:
			...
			if plot_reg_recs:
				...
				plot_reg_recs.plot_history_ids |= plot_reg_recs.plot_history_ids.create({
				...
				plot_reg_recs.partner_id=self.plot_customer.id
				plot_reg_recs.address="%s %s %s %s %s %s" %(self.plot_customer.street or '', self.plot_customer.street2 or '', self.plot_customer.city or '', self.plot_customer.state_id.name or '',self.plot_customer.zip or '',self.plot_customer.country_id.name or '')
				plot_reg_recs.contact_no = self.plot_customer.mobile
